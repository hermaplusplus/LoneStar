...
/proc/browse_messages(type, target_ckey, index, linkless = FALSE, filter, agegate = FALSE)
	...
	if(type == "memo" || type == "watchlist entry")
		...
		while(query_get_type_messages.NextRow())
			...
			var/round_id = query_get_type_messages.item[10]
			output += "<b>"
	...
	if(target_ckey)
		...
		var/datum/db_query/query_get_messages = SSdbcore.NewQuery(
			"SELECT type, secret, id, IFNULL((SELECT byond_key FROM [format_table_name("player")] WHERE ckey = adminckey), adminckey), text, timestamp, server, IFNULL((SELECT byond_key FROM [format_table_name("player")] WHERE ckey = lasteditor), lasteditor), DATEDIFF(NOW(), timestamp), IFNULL((SELECT byond_key FROM [format_table_name("player")] WHERE ckey = targetckey), targetckey), expire_timestamp, severity, round_id FROM [format_table_name("messages")] WHERE type <> 'memo' AND targetckey = :targetckey AND deleted = 0 AND (expire_timestamp > NOW() OR expire_timestamp IS NULL) ORDER BY timestamp DESC",
		...
		while(query_get_messages.NextRow())
			...
			var/round_id = query_get_messages.item[13]
			var/alphatext = ""
			...
			data += "<b>[timestamp] | [server] | Round [round_id] | [admin_key][secret ? " | <i>- Secret</i>" : ""]"
			if(expire_timestamp)
